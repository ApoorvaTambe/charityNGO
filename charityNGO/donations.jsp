...
<div class="logindiv">
  ...
  while(rst.next()){ %>
	 ...
	 <input type="text"  name="r4" placeholder="account no" pattern="[0-9]{16}" title=" 16 digit account number "   required >
	 
	  <label for="fname"> CVV </label>
	 <input type="password"  name="r5" placeholder="CVV" pattern="[0-9]{3}" title=" 3 digit cvv number "  required >
...
if(x!=0){
rst=stmt.executeQuery("select id from donation order by id desc limit 1 ");
if(rst.next()){
int y=stmt.executeUpdate("insert into donationblock(id,datablock) values('"+rst.getString(1)+"','"+currentblock+"')");
}
